...
export class UploadService {
  ...
  getTagCounts(): Observable<any[]> {
    return this.http.get<any[]>(`${this.baseUrl}/schemas/tagCounts`);
  }
  getNameCounts(): Observable<any[]> {
    return this.http.get<any[]>(`${this.baseUrl}/schemas/nameCounts`);
  }

  getDataTablesSorted(order?: string): Observable<any> {
    if (order) {
      return this.http.get<any>(`${this.baseUrl}/tables/sort?order=${order}`);
    } else {
